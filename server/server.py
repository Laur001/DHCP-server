...
class StopServerThread(QThread):
    finished = pyqtSignal()  # Signal to notify when stopping is complete
    ...
    def run(self):
        try:
            ...
            self.server_process.kill()  # Force kill if not stopped within timeout
...
class DHCPServerGUI(QMainWindow):
    def __init__(self):
        ...
        self.active_leases = {}  # Add this as an instance variable to track active leases
    ...
    def stop_server(self):
        if self.server_process:
            ...
            # Run stop process in a separate thread
    ...
    def save_config(self):
        try:
            ...
            # Mark out-of-range IPs as inactive
        ...
        try:
            while True:
                try:
                    ...
                    self.log_message(line)  # Log the line in the GUI
                    if "Sent DHCP ACK to" in line:
                        self.update_lease_table(line)  # Update table for ACK messages
        except Exception as e:
    ...
    def update_lease_table(self, lease_info):
        try:
            # Extract IP and MAC address from the log entry
            if "Sent DHCP ACK to" in lease_info:
                ip = lease_info.split("Sent DHCP ACK to")[1].strip().split()[0]
                mac = self.extract_mac(lease_info)  # You need to implement MAC extraction
                if ip == "(null)":
            ...
            lease_time = self.lease_time.text()  # Lease time from input field
            status = "Active"

            # Check if this MAC already has a lease and assign a new IP
            if mac in self.active_leases:
                old_ip = self.active_leases[mac]
                if old_ip != ip:
                    # Mark the old IP as Inactive
                    self.mark_ip_inactive(old_ip)

            # Update the active lease for this MAC
            self.active_leases[mac] = ip

            # Check for existing IP in the table
    ...
    def mark_out_of_range_leases(self, new_start, new_end):
        try:
            # Convert IP range to integers for comparison
            new_start_int = self.ip_to_int(new_start)
            new_end_int = self.ip_to_int(new_end)

            # Iterate through the lease table
            for row in range(self.lease_table.rowCount()):
                ...
                if ip_item:
                    ...
                    # Check if the IP is out of the new range
    ...
    def extract_mac(self, lease_info):
        try:
            ...
            pass
        return "Unknown"  # Default if MAC isn't found
